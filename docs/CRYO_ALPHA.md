# Cryo Lance animation alpha — 0.5.0-alpha.2
...
- Two counter-rotating textured rune discs per cast, driven by one server event. No per-frame server packets for the discs. Up to 12 crystal rings (216 main crystals plus side splinters) and 24 active cast visuals, visible within 64 blocks; expire on time and clear on disconnect/dimension change.
- The ultimate now uses smoothly growing faceted crystal clusters, rising slivers and expanding frost bands. The matching ceiling sigil is 8.5 blocks above the lower sigil (3 blocks for normal casts). Shatter turns crystals into flying mesh fragments. Damage/freezing remain server-authoritative; the ultimate no longer places ice/snow blocks or emits vanilla explosion sprites. Layered vanilla sounds remain. Its slam animation keyframe is at 0.65 seconds, matching the scheduled tick-13 impact, subject to network/render timing.
...
The disc is a horizontal plane at the cast position. It is depth-tested and can be hidden by uneven terrain; it does not conform to hills. Crystals are visual only and do not block movement. This pass does not add full-body player poses, camera shake or new recorded sounds. Weapon root motion is not player skeleton animation.

## Alpha 2 checks

Compile/package validation is being run for this revision. In-game shader compatibility, crystal blending, ceiling sigil visibility and frame rate need checking. Test early shatter while crystals are growing, normal auto-shatter and three simultaneous casters. The updated alpha must be on the server and every client because it adds a crystal-ring packet.
...
The current host is PowerupStack. Select Minecraft Java, Fabric, and Minecraft 1.21.8. Keep that Minecraft version when choosing a Fabric loader build. The mod is not tied to TickHosting.
...
1. `spacemod-0.5.0-alpha.2.jar` from the successful Actions artifact (extract the ZIP).
...
Do not replace the live installation until those checks pass. For a later deployment, back up/download the world, stop the server, replace the old spacemod JAR, add the matching GeckoLib dependency, and give all players the same files. Do not reinstall the hosting server or change Minecraft versions.
